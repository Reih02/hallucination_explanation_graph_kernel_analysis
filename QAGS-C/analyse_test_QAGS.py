...
def obtain_roc():
    ...
    with open('results_2.csv', newline='') as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            p_hall = float(row[0])  # predicted probability
            label = float(row[1])   # true label
            
            y_true.append(1 if label < 0.6 else 0)
    ...
    plt.title('ROC Curve - QAGS-C', fontsize=18)
    ...
    with open('results_2.csv', newline='') as csvfile:
        ...
        for threshold in np.arange(0, 1, 0.05):
            ...
            for row in reader:
                ...
                if label < 0.6:  # hallucination
                    if p_hall < threshold:
                        metrics["false_negative"] += 1
                    else:
                        metrics["true_positive"] += 1
                        correct += 1
                else:  # non-hallucination
                    if p_hall > threshold:
